...
def play_game():
    ...
    if type(active_player)==ComputerClass.Computer:
        active_player.sort_hand()
        if active_player.choose_cards_to_lay(game) != -1:
            print('próbuję', active_player, active_player.choose_cards_to_lay(game))
            cards = []

            for id in active_player.choose_cards_to_lay(game):
            ...
            return redirect(url_for('play_take_from_stack'))
...
# @app.route('/game/play/computer', methods=['GET','POST'])
# def play_computer():
#     """
#     Turn page
#     :return:
...
def play_take_from_stack():
    ...
    :return:
    """
    print("ciagnie", game.find_active_player())
    if request.form.get('from_stack'):
        game.take_from_stack(int(request.form.get('from_stack')), game.find_active_player())
    else: #gra komputer
        print("ciagnie kompyuter", game.find_active_player())
        game.take_from_stack(3, game.find_active_player())
